...
- `videoToGif` node and backend `video-gif` service now support async conversion to avoid online `504`: frontend submits `POST /api/video-gif/convert-async` then polls `GET /api/video-gif/task/:taskId`, while backend moves `ffprobe + ffmpeg + OSS` processing into a background task with in-memory status tracking and preserves success/failure credits settlement (`frontend/src/components/flow/nodes/VideoToGifNode.tsx`, `backend/src/oss/video-gif.controller.ts`, `backend/src/ai/services/async-video-task.store.ts`).
