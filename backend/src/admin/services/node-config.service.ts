...
export class NodeConfigService {
  ...
  async getAllNodeConfigs() {
    ...
    const overseasNodeKeys = new Set([
      'generate', 'generate4', 'generatePro', 'generatePro4', 'generateReference',
      'midjourney', 'midjourneyV7', 'niji7', 'gptImage2', 'nano2',
      'sora2Video', 'sora2Character', 'viduVideo', 'omniFlashExtVideo',
      'videoAnalyze', 'analysis', 'promptOptimize', 'textChat',
    ]);

    const filteredConfigs = configs.filter((config) => !overseasNodeKeys.has(config.nodeKey));
    const managedModelMap = await this.getManagedModelConfigMap();
  ...
  async initializeDefaultConfigs() {
    const defaultConfigs: NodeConfigDto[] = [
      ...
      //   priceYuan: 6,
      //   description: 'APIMart Omni Flash Ext 视频生成',
      //   metadata: {
      ...
      //       notes: ['参考图最多 3 张，参考视频最多 1 条；有参考视频时不发送 duration'],
      //     },
      //   },
      // },
      {
