...
const ShaderPlaneBackground = () => {
  ...
  const material = useMemo(
    () =>
      new ShaderMaterial({
        uniforms: {
          ...
          iResolution: { value: new Vector3(1, 1, 0) }
        },
        vertexShader: `
    varying vec2 vUv;
    // varying vec2 vUv;
    ...
    //     {
    //         uv.x += 0.6 / i * cos(i * 2.5 * (uv.y-uMouse.y / 100.0) + 0.02 * uTime);
    //         uv.y += 0.6 / i * cos(i * 1.5 * (uv.x-uMouse.x / 100.0) + 0.02 * uTime);
    //     }
    //     gl_FragColor = vec4( 0.01, 0.01 / abs(sin(0.01 * uTime + uv.y * 1.0 + uv.x * 1.0)) , 0.8 / abs(sin(0.01 * uTime - uv.y * 10.0 - uv.x * 10.0)), 1.0);
    //     // * (1.0 - 0.015 / (distance(uv-(uMouse/2.0), vec2(0.5)))) 
    // }
...
#define TAU               (2.0*PI)
#define TIME              mod(0.25*uTime, 30.0)
...
vec3 color(vec3 ro, vec3 rd, vec3 lp, vec4 md) {
  ...
  la.xz *= ROT(TTIME/60.0+2.0*PI);
  // la.xz *= ROT(0.25*TTIME/60.0+PI/2.0);
...
`
  ...
  return <Plane position={[0, 0, 0]} args={[20, 10]} material={material} />;
};

export default ShaderPlaneBackground;
